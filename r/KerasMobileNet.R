#' @title Yin's Version of MobileNet through Keras Framework
...
#' @return NULL
#' @examples
#' @export KerasMobileNet
#'
#' # Define function
KerasMobileNet <- function(
  x = all[, -1],
  y = all[, 1],
  cutoff = 0.9,
  img_rows = 32,
  ...
  model <- application_mobilenet(
    input_shape = input_shape, alpha = alpha,
    depth_multiplier = depth, dropout = dropout, include_top = TRUE,
    weights = NULL, input_tensor = input_img, pooling = TRUE,
    classes = num_classes
  )
  ...
  y_test <- as.matrix(all[(round(cutoff*nrow(all),0)+1):nrow(all), 1]); y_test <- as.numeric(as.character(y_test))
