...
#' @return NULL
#' @examples
#' @export KerasFeatureEngineer
#'
#' # Define function
KerasFeatureEngineer <- function(
  X = X,
  y = y,
  verbatim = TRUE
) {
  ...
  for (i in 1:nrow(X)) {
    ...
    newX <- rbind(
      ...
      c(currGAPvalue_VGG16,
        currGAPvalue_VGG19,
        currGAPvalue_DenseNet121,
        currGAPvalue_DenseNet169,
        currGAPvalue_DenseNet201,
        currGAPvalue_ResNet50,
        currGAPvalue_Xception))
