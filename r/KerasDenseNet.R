#' @title Yin's Version of DenseNet through Keras Framework
...
#' @return NULL
#' @examples
#' @export KerasDenseNet
#'
#' # Define function
KerasDenseNet <- function(
  x = all[, -1],
  y = all[, 1],
  cutoff = 0.9,
  img_rows = 8,
  ...
  model <- application_densenet(
    include_top = TRUE, input_tensor = input_img, dropout_rate = 0.2,
    classes = num_classes, depth = depth, nb_dense_block = nb_dense_block,
    nb_filter = nb_filter)
